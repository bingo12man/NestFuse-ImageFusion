...
def run_demo(nest_model, infrared_path, visible_path, output_path_root, index, f_type):
	...
	for img_fusion in img_fusion_list:
		file_name = 'Snestfuse' + '_' + str(index) + '.png'
...
def main():
	...
	with torch.no_grad():
		...
		for j in range(1): #use three models
			...
			for i in range(21): #no of images
				index = i + 1
				salient(index)
				infrared_path = test_path + 'IRR' + str(index) + '.png'
